     ...
     register FILE *fp;
{
  ...
  if (fp->_file < 0 || _fstat_r (fp->_data, fp->_file, &st) < 0)
  ...
  else
    {
      ...
      if ((st.st_mode & 0170000) == 0100000 && fp->_seek == __sseek)
 {
   fp->_flags |= 0x0400;
 ...
 fp->_flags |= 0x0800;
  ...
  if ((p = _malloc_r (fp->_data, size)) == ((void *)0))
    {
      fp->_flags |= 0x0002;
      fp->_bf._base = fp->_p = fp->_nbuf;
      fp->_bf._size = 1;
