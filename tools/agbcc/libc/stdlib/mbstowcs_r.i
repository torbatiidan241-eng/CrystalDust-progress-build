# 0 "stdlib/mbstowcs_r.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "stdlib/mbstowcs_r.c"
...
# 2 "stdlib/mbstowcs_r.c" 2

size_t
_mbstowcs_r(struct _reent *r , wchar_t *pwcs , const char *s , size_t n , int *state)
...

{
  ...
  while (n > 0)
    {
      bytes = _mbtowc_r (r, ptr, t, __mb_cur_max, state);
      if (bytes == -1)
        return -1;
      else if (bytes == 0)
        return ptr - pwcs;
