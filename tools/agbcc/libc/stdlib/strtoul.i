...
# 122 "stdlib/strtoul.c" 2
# 1 "../ginclude/limits.h" 1
# 123 "stdlib/strtoul.c" 2
# 1 "include/ctype.h" 1
...
unsigned long
_strtoul_r(struct _reent *rptr , const char *nptr , char **endptr , int base)
...

{
 ...
 do {
  c = *s++;
 } while (((_ctype_+1)[(unsigned)(c)]&010));
 if (c == '-') {
  neg = 1;
  c = *s++;
 } else if (c == '+')
 ...
 if (base == 0)
  base = c == '0' ? 8 : 10;
 cutoff = (unsigned long)(2147483647L * 2UL + 1) / (unsigned long)base;
 cutlim = (unsigned long)(2147483647L * 2UL + 1) % (unsigned long)base;
 for (acc = 0, any = 0;; c = *s++) {
  if (((_ctype_+1)[(unsigned)(c)]&04))
   c -= '0';
  else if (((_ctype_+1)[(unsigned)(c)]&(01|02)))
   c -= ((_ctype_+1)[(unsigned)(c)]&01) ? 'A' - 10 : 'a' - 10;
  else
   break;
  if (c >= base)
   break;
